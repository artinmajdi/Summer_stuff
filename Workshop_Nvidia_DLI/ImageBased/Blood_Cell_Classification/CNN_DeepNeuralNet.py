...
x = tf.placeholder('float',[None,HEIGHT2,WIDTH2])
...
def ReadingData():
    ...
    Directory = '/media/groot/Seagate Backup Plus Drive/dataset/new/BloodCell_Images/blood-cells/dataset2-master/images/'
    with open(Directory + 'train_Data.pkl','rb') as output:
        X_train2 = pickle.load(output)

    with open(Directory + 'test_Data.pkl','rb') as output:
        X_test2 = pickle.load(output)

    sz = X_train2.shape
    X_train = np.zeros((sz[0],HEIGHT2 , WIDTH2))
    for i in range(sz[0]):
        im = X_train2[i,...].reshape((sz[1],sz[2]))
        im = rgb2gray(im)
        im2 = cv2.resize(im, dsize=(WIDTH2,HEIGHT2))
        # im2 = resize(im,(1/downsample))
        X_train[i,...] = im2

    sz = X_test2.shape
    X_test = np.zeros((sz[0],HEIGHT2 , WIDTH2))
    for i in range(sz[0]):
        im = X_test2[i,...].reshape((sz[1],sz[2]))
        im = rgb2gray(im)
        # im2 = resize(im,(1/downsample))
        im2 = cv2.resize(im, dsize=(WIDTH2,HEIGHT2))
        X_test[i,...] = im2
    ...
    with open(Directory + 'train_Label.pkl','rb') as output:
        Y_train2 = pickle.load(output)

    with open(Directory + 'test_Label.pkl','rb') as output:
...
def convolutional_network_network(x):
    ...
    x = tf.reshape(x,shape=[-1,WIDTH2,HEIGHT2,1])  # we are reshaping a flat WIDTH*HEIGHT vector to a a*b image
    ##############x = np.resize(x,[a,b])
