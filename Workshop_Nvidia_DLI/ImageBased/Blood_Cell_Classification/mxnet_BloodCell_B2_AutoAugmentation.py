...
def augmentData(data,label,applyDatagen):
    ...
    FullIm_NDArray = mxnet.nd.array(data)# ,ctx=gpu(0)
    FullLb_NDArray = mxnet.nd.array(np.asarray(label)) # ,ctx=gpu(0)
    ...
    datasetOut = gluon.data.DataLoader( BloodCell_dataset , batch_size=batch_size , shuffle=True , num_workers=4)
...
def readingDataset(mode):
    ...
    return FullIm_Gray, FullLb

Train_Im, Train_Label = readingDataset('train')
Test_Im, Test_Label = readingDataset('test')

test_data = augmentData(Test_Im, Test_Label,applyDatagen=0)
...
def evaluate_accuracy(data_iterator, net):
    ...
    return acc.get()[1] , np.asarray(FullPred) , np.asarray(FullLabel)
...
trainer = gluon.Trainer(net.collect_params(), 'sgd', {'learning_rate': .1}) # best: 0.01

# map_characters = {1:'NEUTROPHIL',2:'EOSINOPHIL',3:'MONOCYTE',4:'LYMPHOCYTE'}

epochs = 50
smoothing_constant = .0001
dict_characters = {1:'NEUTROPHIL',2:'EOSINOPHIL',3:'MONOCYTE',4:'LYMPHOCYTE'}

for e in range(epochs):
    if e%5 == 0:
        train_data = augmentData(Train_Im,Train_Label,applyDatagen=1)
    ...
    test_accuracy , test_predictions , test_label = evaluate_accuracy(test_data, net)
    train_accuracy , train_predictions , train_label = evaluate_accuracy(train_data, net)
